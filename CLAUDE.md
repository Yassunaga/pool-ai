...
`agent/tests.py` cobre o **roteamento de mídia do webhook** (POOL-16) com `TestCase` + mocks (`uv run python manage.py test agent`): texto → `enqueue`, áudio/imagem transcritos → `enqueue`, falha/sem-fala → resposta de fallback direta, tipos não suportados → ignorados. Não bate em rede/LLM (tudo mockado). O harness de **evals** em `agent/evals/` (comando `manage.py evals`) é separado: cada cenário roda o grafo real (chama o OpenRouter) num thread isolado com `MemorySaver` — não toca `langgraph_state.sqlite` nem o banco Django. Como bate na API, **custa tokens; rode deliberadamente, não em todo commit** (sai com código 1 se algum cenário reprovar, então serve pra CI manual). Estrutura: `scenarios.py` (roteiros fixos do "cliente"), `assertions.py` (checagens determinísticas: valor citado ≤1x e só após o cliente perguntar, sem hífen, nome/área extraídos, handoff marcado), `judge.py` (LLM-judge pro subjetivo: natural? repetiu pergunta? inventou fato fora do FAQ?), `harness.py` (runner). Mídia transcrita vira texto comum no `enqueue`, então os evals (que rodam o grafo direto, sem passar pelo webhook) já cobrem o conteúdo resultante; falta só um cenário de webhook-level pra mídia ponta-a-ponta.
...
- `POST /api/agent/webhook/evolution/` (`EvolutionWebhookAPIView`) — WhatsApp inbound. Filters out `fromMe`, group (`@g.us`), and `status@broadcast` messages, derives `session_id` from the sender's number (`remoteJid.split('@')[0]`), then **enqueues the text into the Redis debounce buffer** (`agent/services/debounce_service.py::enqueue`) and returns immediately. It does **not** run the graph inline. **Always returns 200**, even on failure, so Evolution doesn't retry and double-run the graph.
  - **Media (POOL-16, `agent/services/media_service.py`):** non-text messages are handled in `_handle_media`. **Audio** is downloaded from Evolution (`/chat/getBase64FromMediaMessage`) and transcribed by a multimodal model (`OPENROUTER_TRANSCRIBE_MODEL`, default `google/gemini-2.0-flash-001` — the default Claude model can't take audio); the transcript is enqueued like normal text. **Image** is interpreted by a vision model (`OPENROUTER_VISION_MODEL`, defaults to the main model) and the description (plus any caption) is enqueued. The transcription/vision happens **inline in the webhook** before enqueue (so it blocks ~1-4s). **Audio/image must never be silent**: on any download/transcription failure, no speech, or unsupported type (video, document), the webhook sends a direct "manda por texto" fallback via `send_text` (bypassing debounce, so it works even if the worker is down). Stickers/reactions/protocol messages are ignored. Common wrappers (ephemeral, view-once, document-with-caption) are unwrapped first.
